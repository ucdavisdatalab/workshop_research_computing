...
at times feel a bit static. But it offers much of the same interactivity that 
...
A complication arises from the fact that directories can branch off from each 
other. Whenever you make a folder within another folder---or a **subdirectory**---
you've created another branch in the tree, one which is at the same time a branch 
of `root` *and* a branch of whatever directory you're currently in. For example, 
this is the structure of the practice exercise we'll be using during our live 
session:
...
though it comes with a trade-off: those `..` sequences are pretty hard to read 
without context, and it's easy to get confused. We'll discuss this more in our 
live session.