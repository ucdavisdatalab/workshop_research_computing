...
or directory. (Remember seeing `.` and `..` in the previous chapter with `ls -lha`?
Your computer keeps track of its directories by placing these two symbols in 
every folder you create.) This shorthand is useful if you're far away from `root`, 
or if, in a coding project, you are using files that are encapsulated in a 
specific directory structure. To continue with the example from above, if you're 
at `computer`:
