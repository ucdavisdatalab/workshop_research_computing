...
Before continuing, it's important to distinguish plain text from **binary** data. 
...
generally it means some kind of data that is *human readable*, which is to say 
it is comprised of a collection of text characters (ASCII, usually). A common 
way to store this data is with a `.txt` file, though code files (`.py`, `.R`) 
and tabular data (`.csv`) also fall under the plain text heading.

Binary data, on the other hand, is comprised of a series of bytes and is not 
human-readable. It is often used for data storage. Common file types include ones 
for images (`.jpg`, `.png`), for sound (`.mp3`), and various executables (`.exe`). 

If you try to open a binary file, for example this image:
...
any such changes would likely cause problems in the file data itself. If we 
...
If you need to get a sense of what kind of data is stored in a file, the `file` 
function is useful. Here's an image:
...
That won't work for us. On the other hand, here's a plain text:
...
The `ASCII text` readout lets us know we can open it with a text editor.
